...
| qsub -N "Sniffles_$(basename $IF)" -cwd -m beas -M S.deBlank@umcutrecht.nl -l h_rt=1:0:0 -l h_vmem=30G -o ${LOGS_FOLDER}/Sniffles_$(basename $IF).out -e ${LOGS_FOLDER}/Sniffles_$(basename $IF).err
			done
